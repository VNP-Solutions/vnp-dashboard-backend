...
export class EmailUtil {
  ...
  async sendInvitationEmail(
    ...
    if (!isExternal) {
      const activationLink = redirectUrl
        ? `${redirectUrl}?email=${encodeURIComponent(email)}&temp_password=${encodeURIComponent(tempPassword)}`
        : ''
      const mailOptions = {
        from: this.configService.get('smtp.email', { infer: true }),
        to: email,
        ...
        html: `
          <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
            ...
            <p style="color: #666;">⏳ This invitation link is valid for <strong>5 days</strong>.</p>
            <p>Click the link below to set up your account and get started:</p>
            ${activationLink ? `<p><a href="${activationLink}" style="display: inline-block; background-color: #007bff; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; margin: 20px 0; font-weight: bold;">Accept Invitation →</a></p>` : ''}
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
          ...
          ${activationLink ? `<p><a href="${activationLink}" style="display: inline-block; background-color: #007bff; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; margin: 20px 0; font-weight: bold;">Activate Account →</a></p>` : ''}
  ...
  /**
   * OTP sent to a super admin authorizing verification of another user's account.
   */
  async sendAdminUserVerifyOtpEmail(
    adminEmail: string,
    otp: number,
    targetUserDisplayName: string,
    targetUserEmail: string
    ...
    const mailOptions = {
      from: this.configService.get('smtp.email', { infer: true }),
      to: adminEmail,
      subject: 'Verify user account confirmation (VNP Solutions)',
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
          <p><strong>${greeting}</strong></p>
          <p>You requested to verify the account for <strong>${targetLabel}</strong> (${targetUserEmail}).</p>
          <p>Use the OTP below to confirm this action in the dashboard:</p>
          <p><strong>Your OTP:</strong></p>
          <div style="background-color: #f4f4f4; padding: 20px; text-align: center; font-size: 32px; font-weight: bold; letter-spacing: 5px; margin: 20px 0;">
            ${otp}
          ...
          <p>This code will expire in <strong>${expiryMinutes} minutes</strong> for your security.</p>
          <p style="color: #666;">Once verified, the user's invitation link will no longer be valid.</p>
          <p style="color: #666;">If you did not request this, you can ignore this email.</p>
          <div style="margin-top: 30px;">
            <p>Stay secure,<br><strong>VNP Solutions Support Team</strong></p>
          </div>
      ...
      `,
      text: `${greeting}\n\nYou requested to verify the account for ${targetLabel} (${targetUserEmail}).\n\nYour OTP: ${otp}\n\nThis code will expire in ${expiryMinutes} minutes for your security.\n\nOnce verified, the user's invitation link will no longer be valid.\n\nIf you did not request this, you can ignore this email.\n\nStay secure,\nVNP Solutions Support Team`
    }

    try {
      const info = await this.transporter.sendMail(mailOptions)
      console.log('✓ Admin user verify OTP email sent:', {
        to: adminEmail,
        messageId: info.messageId
      })
    } catch (error) {
