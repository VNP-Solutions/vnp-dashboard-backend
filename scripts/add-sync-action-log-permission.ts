...
/**
 * Migration: add sync_action_log_permission to existing UserRoles.
 * Super admins get all/all; everyone else starts at view/none (grant via Roles UI).
 */
async function addSyncActionLogPermission() {
  try {
    console.log(
      'Starting migration to add sync_action_log_permission to existing user roles...\n'
    )
    ...
    const allRoles = await prisma.userRole.findMany({
      select: {
        ...
        sync_action_log_permission: true
      }
    })
    ...
    for (const role of allRoles) {
      if (role.sync_action_log_permission) {
        console.log(
          `Role "${role.name}" already has sync_action_log_permission - skipping`
        )
      ...
      if (isSuperAdmin) {
        await prisma.userRole.update({
          where: { id: role.id },
          data: { sync_action_log_permission: SUPER_ADMIN_PERMISSION }
        })
        console.log(
          `Role "${role.name}" is a full super admin - set sync_action_log_permission to all/all`
        )
        superAdminCount++
      } else {
        await prisma.userRole.update({
          where: { id: role.id },
          data: { sync_action_log_permission: NO_ACCESS_PERMISSION }
        })
        console.log(
          `Role "${role.name}" - set sync_action_log_permission to view/none (grant manually if needed)`
        )
        defaultedCount++
      }
    }
    ...
    console.log(
      `Roles already with sync_action_log_permission: ${alreadyHasPermission}`
    )
...
addSyncActionLogPermission().catch(error => {
