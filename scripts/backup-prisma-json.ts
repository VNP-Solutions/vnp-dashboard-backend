...
async function backupDatabasePrisma(options?: BackupOptions) {
  try {
    console.log('========================================')
    ...
    const folderPrefix = options?.folderPrefix ?? 'prisma-backup'
    ...
    fs.writeFileSync(
      path.join(backupDir, 'UserRole.json'),
    ...
    fs.writeFileSync(
      path.join(backupDir, 'Portfolio.json'),
    ...
    fs.writeFileSync(
      path.join(backupDir, 'Property.json'),
    ...
    fs.writeFileSync(
      path.join(backupDir, 'PropertyCredentials.json'),
...
if (require.main === module) {
  backupDatabasePrisma().catch(() => process.exit(1))
}
